...
def read_hypre_binary_matrix(parts: List[str], threshold: float = 0.0) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Tuple[int, int]]:
    ...
    for p in sorted(parts):
        ...
        with open(p, 'rb') as f:
            ...
            if threshold > 0.0:
                ...
                kept = int(mask.sum())
                dropped = loc_nnz - kept
                logging.debug(f"  applying threshold {threshold}: keeping {kept} / {loc_nnz} (dropped {dropped})")
                r = r[mask]
                c = c[mask]
                v = v[mask]
                tot_before += loc_nnz
    ...
    if threshold > 0.0:
        logging.info(f"Threshold {threshold}: kept {tot_after}/{tot_before} (dropped {tot_before - tot_after}) entries across all parts")
...
def main():
    ...
    parser.add_argument('--root', type=str, default='.', help='Root directory containing ls_00000, ls_00001, ... subdirectories')
    parser.add_argument('--pattern', type=str, default='ls_*', help='Subdirectory pattern to discover matrices')
    parser.add_argument('--prefix', type=str, default='IJ.out.A', help='Matrix file prefix inside each subdirectory')
    parser.add_argument('--threshold', type=float, default=0.0, help='Drop entries with |A_ij| <= threshold')
    parser.add_argument('--log', action='store_true', help='Use log(|A_ij|) as color')
    parser.add_argument('--range', type=str, default=None, help='Range as START:END (inclusive). Open ended like :END or START: are allowed')
    parser.add_argument('--start', type=int, default=None, help='First linear system index (alias)')
    parser.add_argument('--end', type=int, default=None, help='Last linear system index inclusive (alias)')
    parser.add_argument('--width', type=int, default=1000, help='Figure width')
    parser.add_argument('--height', type=int, default=900, help='Figure height')
    ...
    parser.add_argument('--save', type=str, default=None, help='Save HTML to this path instead of opening a browser')
    ...
    logging.info(f"Scanning root={args.root!r} pattern={args.pattern!r}")
    subdirs = sorted([d for d in glob.glob(os.path.join(args.root, args.pattern)) if os.path.isdir(d)])
    if not subdirs:
        raise SystemExit(f"No subdirectories found under {args.root!r} matching {args.pattern!r}")
    ...
    for d in subdirs:
        ...
        if args.log:
            cv = np.log(np.maximum(1e-300, np.abs(v))) if v.size else v
        else:
            cv = v
        mats.append((r, c, cv, shape))
        if cv.size:
    ...
    traces = []
    for idx, (r, c, cv, shape) in enumerate(mats):
    ...
    fig.update_layout(
        ...
        title=dict(text=f"Sparsity pattern plot of {os.path.basename(subdirs[0])}/{args.prefix}", font=dict(size=args.tfs), x=0.5, xanchor='center'),
    ...
    steps = []
    for i, (_, _, _, shape) in enumerate(mats):
        vis = [False] * len(mats)
        vis[i] = True
        steps.append(dict(
            method='update',
            args=[
                {'visible': vis},
                {'title': {'text': f"Sparsity pattern plot of {os.path.basename(subdirs[i])}/{args.prefix}", 'font': {'size': args.tfs}, 'x': 0.5, 'xanchor': 'center'},
