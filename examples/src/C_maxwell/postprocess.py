...
"""Render a 3D visualization of the C_maxwell (AMS) solution with PyVista.

Reads the VTK ImageData written by ``maxwell -vtk <base>`` (a single ``.vti`` on
one rank, or a ``.pvti`` master plus per-rank ``.vti`` pieces in parallel) and
produces a publication-quality 3D rendering of the electric-field magnitude |E|.

Styles (``--style``):

* ``iso``     (default) nested translucent isosurfaces (level sets) of the magnitude.
* ``clip``    a cutaway: one octant is removed so the interior field is exposed on the
              cut planes.
...
renders with real depth compositing. The same ``.vti``/``.pvti`` files can also be
...
SOLUTION_LABEL = "|E|"
DEFAULT_OUTPUT = "maxwell_solution_3d.png"
DEFAULT_TITLE = "Maxwell electric-field magnitude"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("input", type=Path,
                   help="VTK input: a .vti (serial) or .pvti master (parallel)")
    ...
    p.add_argument("--field", default="magnitude",
                   help="scalar array to color by (default: magnitude)")
...
def render(args: argparse.Namespace) -> None:
    try:
        ...
        sys.exit("PyVista is required for this script: pip install pyvista\n"
                 "(Alternatively, open the .vti/.pvti file directly in ParaView.)")
    import numpy as np

    pv.OFF_SCREEN = True
    mesh = pv.read(str(args.input))
    field = args.field
    if field in mesh.cell_data:
        arr = np.asarray(mesh.cell_data[field])
    elif field in mesh.point_data:
        arr = np.asarray(mesh.point_data[field])
    else:
    ...
    try:
        ...
        # Anti-aliasing is an optional quality boost that not all VTK/OpenGL
        # backends honor (headless setups in particular); skip it and keep going.
        pass
    try:
        pl.enable_depth_peeling(10, 0.0)  # correct ordering for translucent surfaces
    except Exception:
        # Depth peeling is an optional enhancement for translucent surfaces and may
        # be unsupported depending on the backend; render without it if so.
        pass
    ...
    sbar = dict(title=label, color=fg, title_font_size=38, label_font_size=30, n_labels=5,
                ...
                width=0.07)
    mesh_kw = dict(scalars=field, cmap=args.cmap, clim=clim, scalar_bar_args=sbar)
