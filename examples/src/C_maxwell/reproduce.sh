...
# Mesh-refinement study for the definite Maxwell example. For each grid it
# reports the AMS-preconditioned PCG iteration count (which should stay nearly
# constant -- AMS is uniform in h) and the discretization error against the
# manufactured solution (which should decrease roughly like O(h)).
