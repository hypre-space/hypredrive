         ...
         HYPRE_BigInt **ps)
{
   ...
   return ps[0][c[0]] * gd[1] * gd[2] +
          ps[1][c[1]] * gd[2] * (ps[0][c[0] + 1] - ps[0][c[0]]) +
          ps[2][c[2]] * (ps[0][c[0] + 1] - ps[0][c[0]]) *
             (ps[1][c[1] + 1] - ps[1][c[1]]) +
               ...
               HYPRE_Int Py, HYPRE_Int Pz, DistMesh **msh)
{
   ...
   /* Keep cart ranks consistent with MPI_COMM_WORLD ranks (see lidcavity notes). */
