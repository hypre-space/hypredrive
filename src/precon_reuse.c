...
{                                /* GCOVR_EXCL_BR_LINE */
   if (!starts || !starts->data) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }
...
{                                                   /* GCOVR_EXCL_BR_LINE */
   if (!stats || !(stats->level_active & (1 << 0))) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }

   int timestep_start = stats->level_solve_start[0]; /* GCOVR_EXCL_BR_LINE */
   if (timestep_start < 0 || timestep_start > ls_id) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }
...
{                                                   /* GCOVR_EXCL_BR_LINE */
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (stats->level_current_id[0] <= 0) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }
...
{                                        /* GCOVR_EXCL_BR_LINE */
   ...
   *timestep_start = -1;
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (*timestep_start < 0 || ls_id < *timestep_start) /* GCOVR_EXCL_BR_LINE */
   {
      return 0;
   }
...
{            /* GCOVR_EXCL_BR_LINE */
   ...
   obs->system_index =
      hypredrv->current_system_index >= 0
...
{                           /* GCOVR_EXCL_BR_LINE */
   if (!hypredrv || !state) /* GCOVR_EXCL_BR_LINE */
   {
      return;
   }

   hypredrv_PreconReuseStateReset(state); /* GCOVR_EXCL_BR_LINE */
...
{                              /* GCOVR_EXCL_BR_LINE */
   if (!hypredrv || !decision) /* GCOVR_EXCL_BR_LINE */
   {
      return;
   }
   ...
   {                       /* GCOVR_EXCL_BR_LINE */
      HYPREDRV_LOG_OBJECTF(/* GCOVR_EXCL_BR_LINE */
                           2, hypredrv,
                           "%s adaptive reuse decision: next_ls_id=%d rebuild=%d "
                           "score=%.6g age=%d "
                           "history_points=%d %s",
                           caller ? caller : "reuse", next_ls_id,
                           decision->should_rebuild, decision->score, decision->age,
                           decision->history_points, decision->summary);
   }
   else
   {                       /* GCOVR_EXCL_BR_LINE */
      HYPREDRV_LOG_OBJECTF(/* GCOVR_EXCL_BR_LINE */
                           2, hypredrv,
                           "%s preconditioner reuse decision: next_ls_id=%d rebuild=%d "
                           "%s",
                           caller ? caller : "reuse", next_ls_id,
                           decision->should_rebuild, decision->summary);
   }
}
...
{                      /* GCOVR_EXCL_BR_LINE */
   if (!state || !obs) /* GCOVR_EXCL_BR_LINE */
   {
      return;
   }
   ...
   if (state->count == state->capacity)
   {
      ...
      {
         size_t new_capacity =
            state->capacity ? state->capacity * 2u : 16u;  /* GCOVR_EXCL_BR_LINE */
         if (new_capacity > PRECON_REUSE_MAX_OBSERVATIONS) /* GCOVR_EXCL_BR_LINE */
         {
            new_capacity = PRECON_REUSE_MAX_OBSERVATIONS;
         }
         PreconReuseObservation *new_observations = (PreconReuseObservation *)realloc(
            state->observations,
            new_capacity * sizeof(*new_observations)); /* GCOVR_EXCL_BR_LINE */
         if (!new_observations)                        /* GCOVR_EXCL_BR_LINE */
         {
            hypredrv_ErrorCodeSet(ERROR_ALLOCATION);
   ...
   {
      size_t bootstrap_count = state->count;               /* GCOVR_EXCL_BR_LINE */
      if (bootstrap_count > PRECON_REUSE_BOOTSTRAP_SOLVES) /* GCOVR_EXCL_BR_LINE */
      {
...
{             /* GCOVR_EXCL_BR_LINE */
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (next_ls_id < 0) /* GCOVR_EXCL_BR_LINE */
   {
      next_ls_id = 0;
   }

   int freq = args->frequency;     /* GCOVR_EXCL_BR_LINE */
   if (!args->enabled || freq < 0) /* GCOVR_EXCL_BR_LINE */
   {
      freq = 0;
   }
   /* GCOVR_EXCL_BR_LINE */
   if (args->enabled && args->linear_system_ids &&
       args->linear_system_ids->size > 0) /* GCOVR_EXCL_BR_LINE */
   {
   ...
   if (args->enabled && args->per_timestep)
   {
      ...
      int timestep_period = (freq > 0) ? freq : 1; /* GCOVR_EXCL_BR_LINE */
      if (timestep_idx < 0)                        /* GCOVR_EXCL_BR_LINE */
      {
         return 1;
      }
...
{               /* GCOVR_EXCL_BR_LINE */
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (reduction == PRECON_REUSE_REDUCTION_MEAN &&
       sample->num_solves > 0 && /* GCOVR_EXCL_BR_LINE */
...
{                                                        /* GCOVR_EXCL_BR_LINE */
   if (!stats || level < 0 || level >= STATS_MAX_LEVELS) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }
   /* GCOVR_EXCL_BR_LINE */
   if (!(stats->level_active & (1 << level)) ||
       stats->level_current_id[level] <= 0) /* GCOVR_EXCL_BR_LINE */
   {
      return -1;
   }
...
{                                                            /* GCOVR_EXCL_BR_LINE */
   ...
   int rebuild_start_ls_id = (state->baseline_valid &&
                              state->baseline.system_index >= 0) /* GCOVR_EXCL_BR_LINE */
                                ? state->baseline.system_index   /* GCOVR_EXCL_BR_LINE */
   ...
   else if (component->history.source == PRECON_REUSE_HISTORY_ACTIVE_LEVEL)
   {
      ...
      /* GCOVR_EXCL_BR_LINE */
      for (size_t i = state->count; i > 0 && count < max_samples;
           i--) /* GCOVR_EXCL_BR_LINE */
      {
         const PreconReuseObservation *obs =
            &state->observations[i - 1]; /* GCOVR_EXCL_BR_LINE */
         if (obs->level_ids[component->history.level] !=
             active_level_id) /* GCOVR_EXCL_BR_LINE */
         {
            continue;
         }
         samples[count].num_solves = 1;
   ...
   else if (component->history.source ==
            PRECON_REUSE_HISTORY_COMPLETED_LEVEL) /* GCOVR_EXCL_BR_LINE */
   {
      int const level_count = hypredrv_StatsLevelGetCount(
         stats, component->history.level); /* GCOVR_EXCL_BR_LINE */
      for (int idx = level_count - 1; idx >= 0 && count < max_samples;
           idx--) /* GCOVR_EXCL_BR_LINE */
      {
         LevelEntry entry;
         int        entry_id     = 0;
         int        num_solves   = 0;
         ...
         double     solve_time   = 0.0; /* GCOVR_EXCL_BR_LINE */
         ...
         /* GCOVR_EXCL_BR_LINE */
         if (hypredrv_StatsLevelGetEntrySummary(
                stats, component->history.level, idx, /* GCOVR_EXCL_BR_LINE */
                &entry_id, &num_solves, &linear_iters, &setup_time,
                &solve_time) != /* GCOVR_EXCL_BR_LINE */
             0)                 /* GCOVR_EXCL_BR_LINE */
         {
...
{                                      /* GCOVR_EXCL_BR_LINE */
   ...
   switch (mean->kind)
   {
      ...
      case PRECON_REUSE_MEAN_GEOMETRIC:
      {
         ...
         for (int i = 0; i < count; i++)
         { /* GCOVR_EXCL_BR_LINE */
            double value = values[i] > positive_floor
                              ? values[i]
                              : positive_floor; /* GCOVR_EXCL_BR_LINE */
            sum_logs += log(value);
         }
      ...
      case PRECON_REUSE_MEAN_HARMONIC:
      {
         ...
         return denom > 0.0 ? (double)count / denom : 0.0; /* GCOVR_EXCL_BR_LINE */
      }
      ...
      case PRECON_REUSE_MEAN_POWER:
      {
         ...
         for (int i = 0; i < count; i++)
         {
            double value = values[i];               /* GCOVR_EXCL_BR_LINE */
            if (p <= 0.0 && value < positive_floor) /* GCOVR_EXCL_BR_LINE */
            {
...
{                            /* GCOVR_EXCL_BR_LINE */
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (component->transform.baseline == PRECON_REUSE_BASELINE_WINDOW_MEAN &&
       samples && /* GCOVR_EXCL_BR_LINE */
       count > 0)
   {
      enum
      {
      ...
      int    usable = 0;                /* GCOVR_EXCL_BR_LINE */
      if (count > WINDOW_MEAN_CAPACITY) /* GCOVR_EXCL_BR_LINE */
      {
         fprintf(
            stderr,
      ...
      for (int i = 0; i < count && i < WINDOW_MEAN_CAPACITY; i++) /* GCOVR_EXCL_BR_LINE */
      {
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (!state->baseline_valid) /* GCOVR_EXCL_BR_LINE */
   {
...
{                                       /* GCOVR_EXCL_BR_LINE */
   ...
   { /* GCOVR_EXCL_BR_LINE */
      ...
      double sample_solve = PreconReuseSampleMetricGet(
         sample, PRECON_REUSE_METRIC_SOLVE_TIME,
         component->history.reduction); /* GCOVR_EXCL_BR_LINE */
      if (budget < positive_floor)      /* GCOVR_EXCL_BR_LINE */
      {
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (baseline < positive_floor) /* GCOVR_EXCL_BR_LINE */
   {
      baseline = positive_floor;
   }
   /* GCOVR_EXCL_BR_LINE */
   switch (component->transform.kind) /* GCOVR_EXCL_BR_LINE */
   {
...
{                                              /* GCOVR_EXCL_BR_LINE */
   ...
   {
      /* Mark truncation if it fits and hasn't already been marked. */ /* GCOVR_EXCL_BR_LINE
                                                                        */
      if (used + 4 <= summary_size - 1 && /* GCOVR_EXCL_BR_LINE */
...
{                       /* GCOVR_EXCL_BR_LINE */
   ...
   if (args->guards.max_reuse_solves >= 0 &&           /* GCOVR_EXCL_BR_LINE */
       decision->age >= args->guards.max_reuse_solves) /* GCOVR_EXCL_BR_LINE */
   {
      decision->should_rebuild = 1;
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (args->guards.rebuild_on_solver_failure &&
       state->count > 0 && /* GCOVR_EXCL_BR_LINE */
   ...
   /* GCOVR_EXCL_BR_LINE */
   if (args->guards.rebuild_on_new_timestep && state->count > 0) /* GCOVR_EXCL_BR_LINE */
   {
      int current_timestep_idx   = -1;
      int current_timestep_start = -1; /* GCOVR_EXCL_BR_LINE */
      if (PreconReuseResolveTimestepContext(
             timestep_starts, stats, next_ls_id,                /* GCOVR_EXCL_BR_LINE */
             &current_timestep_idx, &current_timestep_start) && /* GCOVR_EXCL_BR_LINE */
          current_timestep_idx >= 0 &&
          state->last_observation.timestep_index >= 0 &&
          /* GCOVR_EXCL_BR_LINE */ /* GCOVR_EXCL_BR_LINE */
             current_timestep_idx !=
             state->last_observation.timestep_index) /* GCOVR_EXCL_BR_LINE */
      {
         decision->should_rebuild = 1;
         snprintf(decision->summary, sizeof(decision->summary),
   ...
   if (args->guards.rebuild_on_new_level)
   { /* GCOVR_EXCL_BR_LINE */
      for (size_t i = 0; i < args->guards.rebuild_on_new_level->size;
           i++) /* GCOVR_EXCL_BR_LINE */
      {
         int level = args->guards.rebuild_on_new_level->data[i];
         int current_level_id =
            PreconReuseCurrentLevelID(stats, level); /* GCOVR_EXCL_BR_LINE */
         if (current_level_id >= 0 &&
             /* GCOVR_EXCL_BR_LINE */ /* GCOVR_EXCL_BR_LINE */
                current_level_id !=
                state->last_rebuild_level_ids[level]) /* GCOVR_EXCL_BR_LINE */
         {
            decision->should_rebuild = 1;
            snprintf(decision->summary, sizeof(decision->summary),
   ...
   {                                /* GCOVR_EXCL_BR_LINE */
      if (args->guards.max_iteration_ratio > 0.0 &&
          state->baseline_iters > 0.0) /* GCOVR_EXCL_BR_LINE */
      {
         double ratio = (double)state->last_observation.iters /
                        state->baseline_iters;          /* GCOVR_EXCL_BR_LINE */
         if (ratio >= args->guards.max_iteration_ratio) /* GCOVR_EXCL_BR_LINE */
         {
            decision->should_rebuild = 1;
            snprintf(decision->summary, sizeof(decision->summary),
      ...
      /* GCOVR_EXCL_BR_LINE */
      if (args->guards.max_solve_time_ratio > 0.0 &&
          state->baseline_solve_time > 0.0) /* GCOVR_EXCL_BR_LINE */
      {
         double ratio = state->last_observation.solve_time /
                        state->baseline_solve_time;      /* GCOVR_EXCL_BR_LINE */
         if (ratio >= args->guards.max_solve_time_ratio) /* GCOVR_EXCL_BR_LINE */
         {
            decision->should_rebuild = 1;
            snprintf(decision->summary, sizeof(decision->summary),
   ...
   {
      const PreconReuseScoreComponent_args *c =
         &args->adaptive.components[i];                   /* GCOVR_EXCL_BR_LINE */
      if (c->enabled && c->history.max_points > buf_size) /* GCOVR_EXCL_BR_LINE */
      {
   ...
   double *values =
      (double *)malloc((size_t)buf_size * sizeof(*values)); /* GCOVR_EXCL_BR_LINE */
   if (!samples || !values)                                 /* GCOVR_EXCL_BR_LINE */
   {
      free(samples);
      free(values);
      hypredrv_ErrorCodeSet(ERROR_ALLOCATION);
   ...
   for (size_t i = 0; i < args->adaptive.num_components; i++)
   {
      const PreconReuseScoreComponent_args *component =
         &args->adaptive.components[i]; /* GCOVR_EXCL_BR_LINE */
      if (!component->enabled)          /* GCOVR_EXCL_BR_LINE */
      {
         continue;
      }

      int max_points = component->history.max_points; /* GCOVR_EXCL_BR_LINE */
      if (max_points <= 0)                            /* GCOVR_EXCL_BR_LINE */
      {
         max_points = 1;
      }
                                  ...
                                  PreconReuseDecision *decision)
{
   ...
   if (args->policy == PRECON_REUSE_POLICY_ADAPTIVE &&
       args->enabled) /* GCOVR_EXCL_BR_LINE */
   {
   ...
   int should_rebuild = hypredrv_PreconReuseShouldRebuildStatic(
      args, timestep_starts, stats, next_ls_id); /* GCOVR_EXCL_BR_LINE */
   if (decision)                                 /* GCOVR_EXCL_BR_LINE */
   {
      decision->should_rebuild = should_rebuild;
      decision->used_adaptive  = 0;                             /* GCOVR_EXCL_BR_LINE */
      decision->age            = state ? (int)state->count : 0; /* GCOVR_EXCL_BR_LINE */
      snprintf(decision->summary, sizeof(decision->summary),
...
{                         /* GCOVR_EXCL_BR_LINE */
   ...
   if (history->source != PRECON_REUSE_HISTORY_LINEAR_SOLVES && /* GCOVR_EXCL_BR_LINE */
       (history->level < 0 ||
        history->level >= STATS_MAX_LEVELS)) /* GCOVR_EXCL_BR_LINE */
   {
...
{                           /* GCOVR_EXCL_BR_LINE */
   ...
   if (component->metric == PRECON_REUSE_METRIC_SOLVE_OVERHEAD_VS_SETUP &&
       /* GCOVR_EXCL_BR_LINE */                          /* GCOVR_EXCL_BR_LINE */
          component->transform.amortization_window <= 0) /* GCOVR_EXCL_BR_LINE */
   {
      hypredrv_ErrorCodeSet(ERROR_INVALID_VAL);
      hypredrv_ErrorMsgAdd(
   ...
   { /* GCOVR_EXCL_BR_LINE */
      ...
      { /* GCOVR_EXCL_BR_LINE */
                ...
                value, &adaptive->rebuild_threshold)) /* GCOVR_EXCL_BR_LINE */
         {
            ...
            hypredrv_ErrorMsgAdd(                     /* GCOVR_EXCL_BR_LINE */
                                 "Invalid "
                                 "preconditioner.reuse.adaptive.rebuild_threshold: '%s'",
                                 value ? value : "");
            YAML_NODE_SET_INVALID_VAL(child);
            return 0;
         }
      ...
      {                                               /* GCOVR_EXCL_BR_LINE */
                ...
                adaptive->positive_floor <= 0.0) /* GCOVR_EXCL_BR_LINE */
         {
            hypredrv_ErrorCodeSet(ERROR_INVALID_VAL); /* GCOVR_EXCL_BR_LINE */
            hypredrv_ErrorMsgAdd(                     /* GCOVR_EXCL_BR_LINE */
                                 "Invalid preconditioner.reuse.adaptive.positive_floor: "
                                 "'%s'",
                                 value ? value : "");
            YAML_NODE_SET_INVALID_VAL(child);
            return 0;
      ...
      else if (!strcmp(child->key, "components"))
      {
         ...
         PreconReuseScoreComponent_args *components =
            (PreconReuseScoreComponent_args *)calloc(
               count, sizeof(*components)); /* GCOVR_EXCL_BR_LINE */
         if (!components)                   /* GCOVR_EXCL_BR_LINE */
         {
            hypredrv_ErrorCodeSet(ERROR_ALLOCATION);
         ...
         for (YAMLnode *item = child->children; item != NULL; item = item->next)
         {                                   /* GCOVR_EXCL_BR_LINE */
            if (strcmp(item->key, "-") != 0) /* GCOVR_EXCL_BR_LINE */
            {
               continue;
            }
...
{                        /* GCOVR_EXCL_BR_LINE */
       ...
       strcmp(parent->val, "") != 0) /* GCOVR_EXCL_BR_LINE */
   {
      ...
      args->enabled = 1;                                  /* GCOVR_EXCL_BR_LINE */
   ...
   if (args->policy == PRECON_REUSE_POLICY_ADAPTIVE && /* GCOVR_EXCL_BR_LINE */
       args->adaptive.num_components == 0 &&           /* GCOVR_EXCL_BR_LINE */
       !PreconReuseInstallDefaultAdaptiveComponents(&args->adaptive))
   {
      YAML_NODE_SET_INVALID_VAL(parent);
      return;
