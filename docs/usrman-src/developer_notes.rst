...
Use the following checklist when preparing a release.  It spells out the
release gate explicitly because ``build.sh`` is a local convenience script,
not the upstream release contract.  Run the commands from a clean release
branch checkout and keep the final candidate commit identifiable throughout
the process.

1. **Finish the release notes.**

   Add the new version section at the top of ``CHANGELOG``.  Include every
   commit title since the previous release, group related changes by topic,
   keep the entries as small bullets, and include the PR number wherever one
   exists.  Direct commits without a PR number must also be represented.  Set
   the release date to the date the release is published, then review the
   complete diff for omissions.

2. **Bump all release metadata.**

   Preview the changes and then update the version files:

   .. code-block:: console

      $ ./scripts/generate_release.sh <version> --dry-run
      $ ./scripts/generate_release.sh <version>

   The script updates the top-level CMake and Autotools versions, the Sphinx
   fallback version, the Julia package and BinaryBuilder default versions,
   and the Python package and runtime versions.  C++, Fortran, and
   MATLAB/Octave bindings inherit the top-level project version and have no
   separate release version to bump.  ``interfaces/julia/Artifacts.toml`` is
   intentionally not generated by this script; it is populated by the Julia
   artifact workflow in step 5.

   Check the result with ``git diff`` and ``git diff --check``.  Do not use
   the script's ``--tag`` option at this stage: the Julia artifact binding
   commit must be included before the release tag is created.

3. **Commit and publish the release candidate.**

   Review the status, version diff, and recent history, then commit the
   release-note and version changes.  Push ``release-<version>`` and open or
   update the release pull request before dispatching workflows.  Do not use
   a checkout containing generated build products or unrelated untracked
   files for validation or packaging; use a fresh clone or a clean worktree.

4. **Run the complete validation gate.**

   Configure separate build directories for each gate so that coverage,
   sanitizers, and compatibility settings do not interfere with one another.
   The primary build should cover tests, examples, static analysis,
   formatting, compression, and documentation:
   ...
   Run the coverage configuration independently, with analysis disabled:
   ...
   .. code-block:: console
      ...
      $ cmake -S . -B build-coverage -G Ninja \
          ...
          -DHYPREDRV_ENABLE_ANALYSIS=OFF \
          -DHYPREDRV_ENABLE_COMPRESSION=ON
      $ cmake --build build-coverage --parallel
      $ cmake --build build-coverage --target test
      $ cmake --build build-coverage --target coverage

   Repeat the build and test gate against the supported HYPRE versions.  The
   HYPRE 3.1 build should run the normal test target; the HYPRE 2.20 build is
   required to run both labeled suites:

   .. code-block:: console

      $ cmake -S . -B build-hypre-v3.1 -G Ninja \
          -DCMAKE_BUILD_TYPE=RelWithDebInfo \
          -DCMAKE_INSTALL_PREFIX="$PWD/install-hypre-v3.1" \
          -DHYPRE_ROOT=<hypre-v3.1-install> \
          -DHYPREDRV_ENABLE_TESTING=ON \
          -DHYPREDRV_ENABLE_EXAMPLES=ON \
          -DHYPREDRV_ENABLE_COVERAGE=OFF \
          -DHYPREDRV_ENABLE_ANALYSIS=OFF
      $ cmake --build build-hypre-v3.1 --parallel
      $ cmake --install build-hypre-v3.1
      $ cmake --build build-hypre-v3.1 --target test

      $ cmake -S . -B build-hypre-v2.20 -G Ninja \
          -DCMAKE_BUILD_TYPE=RelWithDebInfo \
          -DCMAKE_INSTALL_PREFIX="$PWD/install-hypre-v2.20" \
          -DHYPRE_INCLUDE_DIRS=<hypre-v2.20-install>/include \
          -DHYPRE_LIBRARY=<hypre-v2.20-install>/lib/libHYPRE.a \
          -DHYPREDRV_ENABLE_TESTING=ON \
          -DHYPREDRV_ENABLE_EXAMPLES=ON \
          -DHYPREDRV_ENABLE_COVERAGE=OFF \
          -DHYPREDRV_ENABLE_ANALYSIS=OFF
      $ cmake --build build-hypre-v2.20 --parallel
      $ cmake --install build-hypre-v2.20
      $ ctest --test-dir build-hypre-v2.20 -L unit --output-on-failure
      $ ctest --test-dir build-hypre-v2.20 -L hypredrive --output-on-failure

   Run the GitHub ``CI``, ``Docs``, ``Code Style``, ``Code Analysis``,
   ``Code Coverage``, ``Code Portability``, and ``Fuzzing`` workflows manually
   on the release branch, or have them run through the release pull request.
   Include the language interface jobs when those packages are part of the
   release.  Resolve all failures against the exact candidate commit before
   continuing.

5. **Create the Julia artifact binding before tagging.**

   Dispatch the ``Julia Artifacts`` workflow on ``release-<version>`` with:

   .. code-block:: text

      release_tag=v<version>
      update_artifacts_toml=true

   Wait for the workflow's bot commit updating
   ``interfaces/julia/Artifacts.toml``.  The workflow builds and binds the
   x86_64/aarch64 Linux glibc, x86_64/aarch64 macOS, and x86_64 Windows Julia
   archives; the Linux and macOS archives use MPItrampoline and the Windows
   archive uses Microsoft MPI. Windows ARM64 is not included because the
   selected Microsoft MPI runtime has no matching artifact. Verify that the
   file contains a download URL with exactly
   ``/releases/download/v<version>/`` for each of those platforms.  Treat this
   bot commit as the final candidate; rerun the relevant validation if the
   generated binding changes any tested files.

6. **Tag and publish the release.**

   From the artifact-binding commit, create and push the annotated tag, then
   wait for the Julia artifact release job to upload its assets.  Tag this
   commit manually; do not rerun the version generator with ``--tag`` after
   the version files have already been committed:
   ...
   The tag-triggered ``Julia Artifacts`` workflow creates the GitHub Release
   and uploads all Julia archives.  The tag-triggered ``Python Wheels``
   workflow then uploads the Linux MPICH/OpenMPI, macOS MPICH/OpenMPI, and
   Windows Microsoft MPI wheels to that same release; wait for both
   ``release-assets`` jobs to finish.  Pull-request or manually dispatched
   workflow runs produce Actions artifacts only, not release assets.  Use the
   corresponding ``CHANGELOG`` section as the GitHub Release notes.  Finally,
   verify the tag, source archive, Julia archives, Python wheels, and package
   versions from a clean checkout.

CI artifact topology
~~~~~~~~~~~~~~~~~~~~

Use separate jobs for runner-dependent work.  A GitHub Actions job has one
runner for all of its steps, so steps cannot switch between Linux, macOS, and
Windows or between x86_64 and ARM64.  Use a matrix for native Python wheel
jobs, with one matrix entry per runner and MPI flavor.  The Julia BinaryBuilder
...
than cross-workflow artifact lookups.  This preserves the native runners needed
for Python testing without pretending that one job can change operating systems
or CPU architectures between steps.
