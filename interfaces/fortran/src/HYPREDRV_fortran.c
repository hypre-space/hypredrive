...
uint32_t
HYPREDRV_FortranInputArgsParseYaml(HYPREDRV_t hypredrv, const char *yaml_text)
{
   char *argv[2];
   /* HYPREDRV_InputArgsParse accepts inline YAML text as argv[0]. This depends
    * on the parser treating non-file YAML-looking input as configuration text,
    * not as a path; keep this bridge in sync if that parser heuristic changes. */
   argv[0] = (char *) yaml_text;
   argv[1] = NULL;
   return HYPREDRV_InputArgsParse(1, argv, hypredrv);
}

uint32_t
HYPREDRV_FortranLinearSystemSetMatrixFromCSR(HYPREDRV_t hypredrv,
                                             int64_t    row_start_in,
                                             int64_t    row_end_in,
                                             const int64_t *indptr_in,
                                             int64_t    indptr_len,
                                             ...
                                             int64_t    data_len)
{
   ...
   {
      return HYPREDRV_ErrorInvalidValue("Fortran CSR indptr length does not match row range");
   }

   if (indptr_in[0] != 0 || indptr_in[indptr_len - 1] != col_indices_len)
   {
      return HYPREDRV_ErrorInvalidValue("Fortran CSR input must use normalized indptr[0]=0 and matching nnz length");
   }
   ...
   if (!indptr)
   {
      return HYPREDRV_ErrorInvalidValue("failed to allocate Fortran CSR indptr conversion buffer");
   }
   ...
   if (col_indices_len > 0)
   {
      ...
      {
         free(indptr);
         return HYPREDRV_ErrorInvalidValue("failed to allocate Fortran CSR column conversion buffer");
      }
      for (int64_t i = 0; i < col_indices_len; i++)
      {
         ierr = hypredrv_fortran_bigint_from_i64(col_indices_in[i], "col_indices", &cols[i]);
         if (ierr)
         {
            free(cols);
            free(indptr);
   ...
   ierr = HYPREDRV_LinearSystemSetMatrixFromCSR(hypredrv, row_start, row_end,
                                                indptr, cols, data);
   free(cols);
   free(indptr);
   return ierr;
...
uint32_t
HYPREDRV_FortranLinearSystemSetRHSFromArray(HYPREDRV_t hypredrv,
                                            int64_t    row_start_in,
                                            int64_t    row_end_in,
...
uint32_t
HYPREDRV_FortranLinearSystemGetSolutionLength(HYPREDRV_t hypredrv,
                                              int64_t   *length_out)
{
   HYPRE_BigInt length;
