program test_lifecycle
   ...
   if (.not. c_associated(drv)) error stop 'HYPREDRV_Create returned null handle'
   ...
   if (warmup < 0 .or. num_reps < 0 .or. num_ls < 0 .or. num_variants < 0) then
      error stop 'invalid input-argument getter value'
   end if
   ...
   if (c_associated(drv)) error stop 'HYPREDRV_Destroy did not clear handle'
