...
uint32_t
HYPREDRV_FortranTestCreateVector(MPI_Fint fcomm, int64_t row_start_in,
                                 int64_t row_end_in, HYPRE_Real value,
                                 HYPRE_IJVector *vec)
{
   ...
   uint32_t ierr = (uint32_t) HYPRE_IJVectorCreate(MPI_Comm_f2c(fcomm), row_start,
                                                   row_end, vec);
   if (!ierr)
   {
   ...
   if (!ierr)
   {
      ierr = (uint32_t) HYPRE_IJVectorSetValues(*vec, (HYPRE_Int)nrows, indices,
                                                values);
   }
   if (!ierr)
   {
